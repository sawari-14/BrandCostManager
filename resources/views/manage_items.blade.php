<x-app-layout>
    ...
    <div class="container my-3">
        ...
        <div class="row rounded border text-center" style="background-color: white;">
            ...
            <form action="/manage_item_detail/{{ $item['id'] }}" method="get" class="col border-end" style="max-width:100px;">
                @csrf
                <input type="hidden" name="id" value="{{ $item['id'] }}">
                <button type="submit" class="btn btn-outline-success">詳細</button>
            </form>
            <form action="/manage_delete_item/{{ $item['id'] }}" method="get" class="col" style="max-width:100px;" onSubmit="return checkDelete()">
                @csrf
                <input type="hidden" name="id" value="{{ $item['id'] }}">
